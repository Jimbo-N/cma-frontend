          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="设备计量证书">
                <el-button type="text" v-if="props.row.certificate" @click="download(props.row.certificate.id)">{{
                  props.row.certificate.fileName }}</el-button>
              </el-form-item>
              <el-form-item label="仪器操作规程">
                <el-button type="text" v-if="props.row.rules" @click="download(props.row.rules.id)">{{
                  props.row.rules.fileName }}</el-button>
              </el-form-item>
              <el-form-item label="仪器授权使用人员">
                <el-button type="text" v-if="props.row.people" @click="download(props.row.people.id)">{{
                  props.row.people.fileName }}</el-button>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
      ...
      <el-dialog :title="tittle" :rules="rules" :visible.sync="open" width="700px" append-to-body>
        <el-form ref="addForm" :model="form" :rules="rules" label-width="180px">
          ...
          <el-form-item label="设备计量证书" prop="certificate">
            ...
            <el-upload class="upload-demo" :auto-upload="false" action="http" :on-remove="handleRemoveCertificate"
              :before-remove="beforeRemove" :on-change="handleCertificate" multiple :on-exceed="handleExceed" :limit="1"
              :file-list="cerList">
          ...
          <el-form-item label="仪器操作规范">
            <el-upload class="upload-demo" :auto-upload="false" action="http" :on-remove="handleRemoveRules"
              :before-remove="beforeRemove" :on-change="handleRules" multiple :on-exceed="handleExceed" :limit="1"
              :file-list="rulesList">
              <el-button size="small" type="primary">点击上传</el-button>
              <!--              <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>-->
            </el-upload>
          ...
          <el-form-item label="仪器授权使用人员一览表">
            <el-upload class="upload-demo" :auto-upload="false" action="http" :on-remove="handleRemovePeople"
              :before-remove="beforeRemove" :on-change="handlePeople" multiple :on-exceed="handleExceed" :limit="1"
              :file-list="peoList">
              <el-button size="small" type="primary">点击上传</el-button>
              <!--              <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>-->
            </el-upload>
